...
int main() {
  ...
  while (window.isOpen()) {  // main loop
    ...
    while (window.pollEvent(event)) {  // event loop
      ...
      switch (event.type) {
        ...
        case (sf::Event::MouseMoved):
          ...
          if (mouse_move_x != 0 || mouse_move_y != 0) FrameStill = 1;
    ...
    camera_position.x += cos(res_mouse_x) * camera_movement.x -
                         camera_movement.y * sin(res_mouse_x);
    camera_position.y += sin(res_mouse_x) * camera_movement.x +
    ...
    if (ImGui::BeginCombo(
            "Sphere material",
            current_status))  // The second parameter is the label previewed
                              // before opening the combo.
    {
      for (int n = 0; n < IM_ARRAYSIZE(status); n++) {
        bool is_selected =
            (current_status ==
             status[n]);  // You can store your selection however you want,
    ...
    shader.setUniform("u_seed1",
                      sf::Vector2f((float)dist(e2), (float)dist(e2)) * 999.0f);
    shader.setUniform("u_seed2",
                      sf::Vector2f((float)dist(e2), (float)dist(e2)) * 999.0f);
